...
TEST(RobotHurtProcessor, test_robot_hurt_processor)
{
  ext_robot_hurt_t robot_hurt;
  robot_hurt.armor_id = 0x01;
  robot_hurt.hurt_type = 0x01;
  rmoss_base::FixedPacket64 packet;
  packet.load_data(static_cast<uint8_t>(rm_base::RecvID::ROBOTHURT), 1);
  packet.load_data(robot_hurt, 2);
  packet.set_check_byte(rm_base::Get_CRC8_Check_Sum(packet.buffer() + 1, 61, rm_base::CRC8_INIT));

  rclcpp::init(0, nullptr);
  auto node = rclcpp::Node::make_shared("test_robot_hurt_frame");
  EXPECT_TRUE(
    rm_base::ProcessFactory::create(
      static_cast<uint8_t>(rm_base::RecvID::ROBOTHURT),
      node.get()));
  EXPECT_TRUE(
